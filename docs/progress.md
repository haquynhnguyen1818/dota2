...
- `load_heroes_roles.py` — loads the hand-curated hero → Carry/Midlane/Offlane/Supports Google Sheet (`HERO_ROLE_SHEET_CSV`, fetched as CSV via `export?format=csv`, link-readable) → `roles_csv_import`, `hero_roles_csv_import`. Replaced `data/hero_role.csv` (deleted) so the sheet is the only place to edit roles; edits take effect on the next loader run, not live.
